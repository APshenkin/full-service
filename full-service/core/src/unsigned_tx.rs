...
use mc_transaction_core::{
    ...
    ring_signature::{KeyImage, Scalar}, TokenId, tx::{TxIn, TxOut},
};
use mc_transaction_std::{EmptyMemoBuilder, InputCredentials, ReservedSubaddresses, TransactionBuilder};
use rand::{CryptoRng, RngCore};
use rand::prelude::ThreadRng;
use serde::{Deserialize, Serialize};

use crate::error::WalletTransactionBuilderError;
use crate::fog_resolver::FullServiceFogResolver;
use crate::models::tx_proposal::{InputTxo, OutputTxo, TxProposal};
use crate::models::tx_signing_data::TxSigningData;
use crate::transaction_memo::TransactionMemo;
...
impl UnsignedTx {
    pub fn get_signing_data(
        ...
        view_account_key: Option<&ViewAccountKey>,
        change_address_index: Option<u64>,
        fog_resolver: FullServiceFogResolver,
    ) -> Result<TxSigningData, WalletTransactionBuilderError> {
        ...
        let (
            ...
            payload_txos,
            change_txos
        ) = self.build_transaction(account_key, view_account_key, change_address_index, fog_resolver, &mut rng)?;

        let signing_data = transaction_builder.get_signing_data(&mut rng)?;

        Ok(TxSigningData {
            tx_prefix: signing_data.tx_prefix,
            signing_data: signing_data.signing_data,
            rings: signing_data.rings,
            input_txos,
            payload_txos,
            change_txos,
        })
    ...
    pub fn sign(
        ...
        let (
            ...
            payload_txos,
            change_txos
        ) = self.build_transaction(Some(&account_key),
                                   None,
                                   None, fog_resolver,
                                   &mut rng)?;
    ...
    fn build_transaction(
    ...
    ) -> Result<(TransactionBuilder<FullServiceFogResolver>, Vec<InputTxo>, Vec<OutputTxo>, Vec<OutputTxo>), WalletTransactionBuilderError> {
        // Create transaction builder.
        let fee = Amount::new(self.fee, TokenId::from(self.fee_token_id));

        let mut transaction_builder = match account_key {
            Some(key) =>
                TransactionBuilder::new_with_box(
                    self.block_version,
                    fee,
                    fog_resolver,
                    self.memo.memo_builder(key)),
        ...
        self.inputs_and_real_indices_and_subaddress_indices
        {
            ...
            let view_private_key = match account_key {
                Some(key) => Ok(key.view_private_key()),

                _ => match view_account_key {
                    Some(view_key) => Ok(view_key.view_private_key()),

                    _ => Err(WalletTransactionBuilderError::KeyError(KeyError::InternalError))
                }
            }?;

            let onetime_private_key = match account_key {
                Some(key) => recover_onetime_private_key(
                    &tx_public_key,
                    view_private_key,
                    &key.subaddress_spend_private(subaddress_index),
                ),

                // insert empty onetime_private_key if there is no spend_private_key
                _ => {
                    let dummy: [u8; 32] = [0; 32];
                    RistrettoPrivate::try_from(&dummy).unwrap()
                }
            };
            ...
            let tx_out = &tx_in.ring[real_index as usize];
            let (amount, _) = decode_amount(tx_out, view_private_key)?;
        ...
        Ok((transaction_builder, input_txos, payload_txos, change_txos))
    }
}
...
pub fn decode_amount(
    ...
    match &tx_out.masked_amount {
        Some(masked_amount) => Ok(masked_amount.get_value(&shared_secret)?),

        _ => Err(WalletTransactionBuilderError::NoMaskedAmount)
    }
}
...
fn add_change_output<RNG: CryptoRng + RngCore>(
    ...
    let reserved_subaddresses = match change_address_index {
        Some(index) => match account_key {
            Some(key) =>
                Ok(ReservedSubaddresses::from_subaddress_index(key,
                                                            change_address_index,
                                                            change_address_index)),

            _ => match view_account_key {
                Some(view_key) => Ok(ReservedSubaddresses {
                    primary_address: view_key.subaddress(index),
                    change_subaddress: view_key.subaddress(index),
                    gift_code_subaddress: view_key.subaddress(index),
                }),
        ...
        _ => match account_key {
            Some(key) => Ok(ReservedSubaddresses::from(key)),

            _ => match view_account_key {
                Some(view_key) => Ok(ReservedSubaddresses {
                    primary_address: view_key.default_subaddress(),
                    change_subaddress: view_key.change_subaddress(),
                    gift_code_subaddress: view_key.gift_code_subaddress(),
                }),
                _ => Err(WalletTransactionBuilderError::KeyError(KeyError::InternalError))
            }
        }
