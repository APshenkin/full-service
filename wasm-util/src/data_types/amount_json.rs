...
impl From<&mc_transaction_core::Amount> for AmountJSON {
    fn from(src: &mc_transaction_core::Amount) -> Self {
        Self::new(src.value, src.token_id)
    }
}

impl TryFrom<&AmountJSON> for mc_transaction_core::Amount {
    type Error = String;

    fn try_from(src: &AmountJSON) -> Result<Self, String> {
        Ok(Self {
            value: src
                .value
                .parse::<u64>()
